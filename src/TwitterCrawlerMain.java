...
public class TwitterCrawlerMain {
    ...
    private final static String CONSUMER_KEY = "xnDE5CcSmD7V5ZjtiHCUjiep9";
    private final static String CONSUMER_KEY_SECRET = "0OAEjLRpPAkSilFf5ZzbpGlYDrCDMIeUm58Fv4ysTgYALvdA39";
    ...
    /* To get info from the not annotated tweets */
    	...
    	do {
			System.out.print("Enter the hashtags. Enter @ to stop: ");
			String inputStr = sc.next();
			if(inputStr.equals("@")) {
				break;
			}
			else {
				hashTags.add(inputStr);
			}

		}while(true);
    	
    	do {
			System.out.print("Enter the amount of tweets for each hashtags:  ");
			try {
				count = sc.nextInt();
				break;
    	...
    	SearchHashtag s = new SearchHashtag();
    	s.run(hashTags, count);
    }
}
